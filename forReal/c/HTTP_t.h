#ifndef __CLASS_HTTP_T_HEADER
#define __CLASS_HTTP_T_HEADER

struct HTTP_t_ClassStruct {
  struct java_lang_Object_ClassStruct *super;
  struct HTTP_t_MethodTblStruct *methodTbl;
...
typedef GC_STRUCT_BEGIN(HTTP_t_ClassStatic)
  /* static fields go here, from this class and its ancestors */
  volatile GC_STRUCT_REF(JByteArray,_HTTP_t_CRLF);
GC_STRUCT_END(HTTP_t_ClassStatic) HTTP_t_ClassStatic;

typedef GC_STRUCT_BEGIN(HTTP_t)
  GC_STRUCT_VAR(struct HTTP_t_ClassStruct *,class);
  ...
  volatile GC_STRUCT_REF(Monitor,_HTTP_t_mon);
  volatile GC_STRUCT_VAR(JInt,_HTTP_t_port);
GC_STRUCT_END(HTTP_t) HTTP_t;

struct HTTP_t_MethodTblStruct {
  ...
  void (*run)(void **__eFlag__, GC_PARAM(HTTP_t,this));
  ...
  void (*yield)();
  GC___REF(struct java_lang_String) (*getLine_java_io_InputStream)(GC_PARAM(java_io_InputStream,s));
  void (*putLine_java_io_OutputStream_java_lang_String)(GC_PARAM(java_io_OutputStream,s),GC_PARAM(java_lang_String,str));
};
extern struct HTTP_t_MethodTblStruct HTTP_t_MethodTbl ;
